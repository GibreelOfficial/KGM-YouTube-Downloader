...
class MainContentView(QWidget):
    def __init__(self, parent=None):
        ...
        self.progress_bar.setObjectName("detailProgressBar")
    ...
    def setup_top_control_panel(self):
        ...
        self.detail_path = QLabel("/Users/gibreal/Downloads")
    ...
    def add_record_to_table_ui(self, record):
        ...
        delete_btn.clicked.connect(lambda: self.delete_record_by_item_trigger(tracking_item))
    ...
    def update_active_progress(self, percent_value, status_text=None):
        ...
        if self.table.rowCount() > 0:
            ...
            if isinstance(row_widget, QProgressBar):
                ...
                if percent_value > 0 and row_widget.property("statusClass") != "active":
                    row_widget.setProperty("statusClass", "active")
                    row_widget.style().unpolish(row_widget)
                    row_widget.style().polish(row_widget)
                
            if speed != "---":
                self.table.setItem(active_row, 5, QTableWidgetItem(speed))
            if eta != "---":
                self.table.setItem(active_row, 6, QTableWidgetItem(eta))
    ...
    def update_row_status(self, row_index, state_string):
        ...
        if state in ["complete", "done"]:
            ...
            status_label.setStyleSheet("""
                color: #8bc34a; border: 1px solid #8bc34a; border-radius: 10px; padding: 2px 8px; background: transparent;
            """)
        elif state in ["queued", "quede"]:
            status_label.setText("queued")
            status_label.setStyleSheet("""
                color: #03a9f4; border: 1px solid #03a9f4; border-radius: 10px; padding: 2px 8px; background: transparent;
            """)
        elif state == "failed":
            status_label.setText("failed")
            status_label.setStyleSheet("""
                color: #e51c23; border: 1px solid #e51c23; border-radius: 10px; padding: 2px 8px; background: transparent;
            """)
        elif state in ["downloading", "active"]:
            status_label.setText("active")
            status_label.setStyleSheet("""
                color: #ff9800; border: 1px solid #ff9800; border-radius: 10px; padding: 2px 8px; background: transparent;
            """)
